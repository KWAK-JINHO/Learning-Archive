...
- 기본정보, 부가정보 등 업무의 핵심 기능과 밀접

## 행위 엔티티(업무상세)

- 상세/내역: 중요엔티티를 분류 또는 세분화
- 상태: 진행상황, 단계
- 이력: 과거 상태변화, 처리내역

## 엔티티 도출 및 식별

**쪼갤 수 있는데까지 쪼개고 나중에 합치자**

- 단어 및 용어의 통일
    - 논리 설계 단계에서는 속성명을 매우 자세하게 적어야 한다.

# 관계 도출 및 정의

## 관계 정의

- 엔티티 간의 관계수, 선택성, 식별자 상속을 정의

# 속성 도출 및 정의

## 속성명 부여

- 속성을 가장 명확히 나타낼 수 있는 명칭 부여
  **컬럼이 많아질수록 단어와 단어의 분리가 어려워진다. 이 때 단어와 단어 사이에 언더바로 구분해 주면 나중에 편함**

## 인조식별자를 정의하는 경우

1. 엔티티를 통합할 때
2. 데이터 모델을 단순화 or 물리 모델로 변환했을 때 편의성 측면에서
3. 개인정보 암호화 대상에 해당할 때
4. 본질 식별자 데이터가 들어오지 않은 상태에서 업무처리를 위해서

## 옵셔널리티(Not Null) 지정

- 해당 속성값 Not Null 여부 결정
...
- Join 할 때 사용여부 체크가 중요하다. 참조되고 있는 코드가 삭제되거나 하면 무결성이 깨질 수 있기 때문에

### 코드 부여

- 분류형: 대/중/소 세가지 분류형태 계층형 코드체계, 코드 자리수 의미 부여
- 예시) -> 01: 농업 001: 작물, 재배업 0111: 곡물 및 식량작물 재배업 012: 축산업
- 자신의 코드가 부모코드 포함, 리스트업에 유리하다.
